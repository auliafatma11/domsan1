<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" type="image/x-icon"  sizes="64x64" href="logo/dom-san.png" />
    <?php include "parts/head.php";  ?>
    <link rel="stylesheet" href="css/style-landing.css">
    <title>Landing</title>
</head>
<body id="page-top">
        <!-- Masthead-->
        <header class="masthead">
            <div class="container px-4 px-lg-5 h-100">
                <div class="row gx-4 gx-lg-5 h-100 align-items-center justify-content-center text-center">
                    <div class="col-lg-8 align-self-end">
                        <h1 class="text-white font-weight-bold">welcome to our application </h1>
                        <hr class="divide" />
                    </div>
                    <div class="col-lg-8 align-self-baseline">
                        <p class="text-white-75 mb-5">Dom-$an akan hadir untuk membantu pemantauan keuangan anda.</p>
                        <a class="btn btn-primary btn-xl" href="login.php">Login</a> <br><br>
                        <a class="btn btn-primary btn-xl" href="https://fahruroziii.github.io/panduan-aplikasi/" target="blank">Panduan Aplikasi</a>
                    </div>
                </div>
            </div>
        </header>
    </body>
</html>


